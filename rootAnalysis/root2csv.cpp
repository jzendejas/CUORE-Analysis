...
void writeCSV(TTree* tree) {
	...
	//find where the directory /root/ is in the file name
	size_t index = filename.find("root");
	filename.replace(index,4,"csv");

	end = filename.find(".root");
	subfilename = filename.substr(0,end) + ".csv";
	ofstream myfile;
	myfile.open(subfilename.c_str());
	cout << "The new file name is: " << subfilename << endl;
	for (int i = 0; i < dim; i++) {
...
void root2csv(TString run) {

	const char *ext = ".root";
	TString dirname = "./data/root/";
	...
	if (files) { 
		...
		cout << "Okay there are files." << endl;
		while ((file=(TSystemFile*)next())) {
			fname = file->GetName();
			cout << "Filename is : " << fname << endl;
			
			if (!file->IsDirectory() && fname.EndsWith(ext) && fname.Contains(run)) {
				TFile *hfile = new TFile(dirname + fname);
				TTree* tree = nullptr;
	...
	} else { cout << "There are no files." << endl; }
}
